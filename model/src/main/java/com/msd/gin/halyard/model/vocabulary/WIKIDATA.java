...
public final class WIKIDATA implements Vocabulary {
	...
	static final class EntityNamespace extends AbstractIRIEncodingNamespace {
		...
		public ByteBuffer writeBytes(String localName, ByteBuffer b) {
			...
			if (pos != -1) {
				...
				b = ByteUtils.ensureCapacity(b, 2 + len);
				b.put((byte) '-');
				byte[] subId = ByteUtils.fromString(localName.substring(pos+1));
				b.put((byte)subId.length);
				b.put(subId);
			}
			b = ByteUtils.ensureCapacity(b, 1);
			b.put((byte) localName.charAt(0));
			return ByteUtils.writeCompressedInteger(localName.substring(1, pos != -1 ? pos : localName.length()), b);
		}
		...
		public String readBytes(ByteBuffer b) {
			...
			if (type == '-') {
				...
				b.get(subId);
				suffix = "-" + ByteUtils.toString(subId);
				type = (char) b.get();
			} else {
				suffix = "";
			}
	...
	static final class StatementNamespace extends AbstractIRIEncodingNamespace {
		...
		public ByteBuffer writeBytes(String localName, ByteBuffer b) {
			...
			b = ByteUtils.ensureCapacity(b, 1 + Long.BYTES + Long.BYTES);
			b.put((byte) localName.charAt(0));
			b.putLong(uuid.getMostSignificantBits());
			b.putLong(uuid.getLeastSignificantBits());
			return ByteUtils.writeCompressedInteger(localName.substring(1, uuidPos), b);
		}
