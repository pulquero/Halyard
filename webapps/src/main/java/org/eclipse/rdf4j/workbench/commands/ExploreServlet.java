...
public class ExploreServlet extends TupleServlet {
	...
	protected ResultCursor processResource(final RepositoryConnection con, final TupleResultBuilder builder,
		...
		// Halyard changes
		if (resource) {
			exportNested(con, builder, cursor, (Resource) value, null, null);
			logger.debug("After nested subject, total = {}", cursor.getTotalResultCount());
		}
		if (value instanceof IRI) {
			exportNested(con, builder, cursor, null, (IRI) value, null);
			logger.debug("After nested predicate, total = {}", cursor.getTotalResultCount());
		}
		if (value != null) {
			exportNested(con, builder, cursor, null, null, value);
			logger.debug("After nested object, total = {}", cursor.getTotalResultCount());
		}
	...
	// Halyard changes
	private void exportNested(RepositoryConnection con, TupleResultBuilder builder, ResultCursor cursor, Resource subj,
			IRI pred, Value obj, Resource... context)
			throws RDF4JException, MalformedQueryException, QueryEvaluationException {
		String query = "CONSTRUCT {?t ?u ?v} WHERE {bind(<< ?s ?p ?o >> as ?t) ?t ?u ?v}";
		GraphQuery gq = con.prepareGraphQuery(query);
		if (subj != null) {
