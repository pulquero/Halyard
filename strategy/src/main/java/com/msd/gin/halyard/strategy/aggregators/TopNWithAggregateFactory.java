...
public final class TopNWithAggregateFactory implements AggregateFunctionFactory {
	...
	public AggregateFunction buildFunction(Function<BindingSet, Value> evaluationStep) {
		return new TopNWithAggregateFunction(evaluationStep);
	}
	...
	public static final class TopNWithAggregateFunction extends ThreadSafeAggregateFunction<TopNCollector,Value> {
		TopNWithAggregateFunction(Function<BindingSet, Value> evaluator) {
			super(evaluator);
		}

		@Override
		public void processAggregate(BindingSet bs, Predicate<Value> distinctPredicate, TopNCollector col) {
			Value v = evaluate(bs);
	...
	private static final class TopNCollector implements AggregateCollector {
