    ...
    static final class SummaryMapper extends TableMapper<ImmutableBytesWritable, LongWritable>  {
        ...
        private Set<IRI> queryForClasses(Value instance) throws IOException {
            if (instance instanceof Resource) {
                ...
                Resource subj = (Resource) instance;
                Scan scan = HalyardTableUtils.scan(subj, RDF.TYPE, null, null);
                try (ResultScanner scanner = table.getScanner(scan)) {
                    for (Result r : scanner) {
                        Statement st = HalyardTableUtils.parseStatement(r, subj, RDF.TYPE, null, null, SVF);
        ...
        protected void map(ImmutableBytesWritable key, Result value, Context output) throws IOException, InterruptedException {
            if (random.nextInt(decimationFactor) == 0) {
                statementChange(HalyardTableUtils.parseStatement(value, SVF));
            }
            if (++counter % 10000 == 0) {
